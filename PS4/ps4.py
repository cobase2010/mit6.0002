...
class Patient(object):
    ...
    def update(self):
        ...
        pop_density = float(len(new_bacteria)) / self.max_pop
...
def calc_pop_avg(populations, n):
    ...
    """
    sum2 = sum(x[n] for x in populations)
    return sum2 / float(len(populations))
...
def simulation_without_antibiotic(num_bacteria,
    ...
    """
    timesteps = 300
    populations = [[0 for i in range(timesteps)] for j in range(num_trials)]
...
# mean, ci_95 = calc_95_ci(populations, 299)
# print("95% confidence interval:",  , mean, "+/-", ci_95)
...
class TreatedPatient(Patient):
    ...
    def get_resist_pop(self):
        ...
        """
        return len([bacterium for bacterium in self.bacteria if bacterium.get_resistant()])
    ...
    def update(self):
        ...
        pop_density = float(len(surviving_bacteria)) / self.max_pop
...
def simulation_with_antibiotic(num_bacteria,
    ...
    timesteps = 300
    populations = [[0 for i in range(timesteps)] for j in range(num_trials)]
    resistant_pop = [[0 for i in range(timesteps)] for j in range(num_trials)]
    for i in range(num_trials):
        bacteria = []
        for n in range(num_bacteria):
    ...
    make_two_curve_plot(range(timesteps),
        ...
        'total pop', 'resistant pop', 'Timestep', 'Average Population', 'With Antibiotic')
...
1. What happens to the total population before introducing the antibiotic
   Population grows steadily. A total pop grows to the max, B total pop raises then drops due to much smaller birth_rate.
2. What happens to the resistant bacteria population before introducing the antibiotic?
   A resistant bacteria population rises then drops due to much higher death_rate for resistant bacteria

3. What happens to the total population after introducing the antibiotic?
   All non-resistant bacteria were killed after antibiotic, so total pop == resistant pop

4. What happens to the resistant bacteria population after introducing the antibiotic
   A resistant pop stays at certain level while B resistant pop drops to zero due to smaller birth rate.
