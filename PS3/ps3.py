...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        tiles is a 2 dimensional list that stores height of list of width wide with dirt_amount of dirt on each

        """
        self.width = width
        self.height = height
        
        self.tiles = [] # can be either a dictionary or 2D list
        for row in range(width):
            self.tiles += [[dirt_amount] * int(height)]
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tiles[x][y] < 0: # capacity is greater than the amount of dirt on the tile
            self.tiles[x][y] = 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return sum(x.count(0) for x in self.tiles)
...
class Robot(object):
    ...
    def get_robot_speed(self):
        """
        Returns: a float d giving the speed of the robot
        """
        return self.speed
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.get_room().is_position_valid(new_pos):
            # move the robot to new position
            super().set_robot_position(new_pos)
            # clean at the current position
            self.get_room().clean_tile_at_position(new_pos, self.get_capacity())
        else:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
                  robot_type, room_type = EmptyRoom, show_anim = False):
    """
    ...
    for trial in range(num_trials):
        ...
        sum += time_steps
        # print("Trial:", trial, "timestep", time_steps, "Sum", sum)
    if show_anim:
        anim.done()
...
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 5, 5, 1, 1.0, 100, StandardRobot, EmptyRoom)))
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.8, 50, StandardRobot)))
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.9, 50, StandardRobot)))
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot)))
# print ('avg time steps: ' + str(run_simulation(3, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot)))
# print ('avg time steps: ' + str(run_simulation(3, 1.0, 1, 10, 10, 3, 1.0, 1, StandardRobot, FurnishedRoom, True)))
...
#       of a 20x20 room?
#   StandardRobot takes fewer average steps to clean the room, but the differences
#   get smaller as the number of robots in the room increases.
#
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#   FaultyRobot takes proportional more steps to clean as the aspect ratio increases with the 
